-- Issue 1: Remove member auth emails from onboarding interest events
-- Issue 23 (partial): Add stable error code hints to complete_onboarding
--
-- 1. NULL out email on existing onboarding_looking_for interest events
-- 2. Recreate complete_onboarding to pass NULL email for onboarding events
-- 3. Add stable application error codes via the hint field on all raise exceptions

-- Temporarily drop the append-only trigger to allow the backfill UPDATE.
drop trigger if exists prevent_interest_event_mutation on private.interest_events;

update private.interest_events
  set email = null
  where source = 'onboarding_looking_for';
...
-- Recreate complete_onboarding with NULL email for onboarding events and
-- stable error code hints for the frontend error mapper.
...
begin
  ...
  if v_clean_username is null or v_clean_username !~ '^[a-z]{3,24}$' then
    raise exception 'Username must be 3-24 letters only'
...
comment on function public.complete_onboarding(text, text, text, text, text[]) is
  'Claims a normalized letters-only username under a global transaction lock and completes Greater Lansing onboarding once. Onboarding interest events do not store member auth emails.';

...
export default function PublicCatalogLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen flex-col bg-stone-50">
      <header className="sticky top-0 z-40 border-b border-gray-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex min-h-14 w-full max-w-2xl flex-wrap items-center justify-between gap-2 px-4 py-2">
          ...
          <nav className="flex items-center gap-1">
            <NavLink to="/stores" className={({ isActive }) => `rounded-lg px-3 py-1.5 text-sm font-medium ${isActive ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-100'}`}>
              Stores
            </NavLink>
      ...
      <footer className="border-t border-gray-200 py-5 text-center text-xs text-gray-500">

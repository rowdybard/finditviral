...
import type { Trend, Bounty, Sighting } from '../types/database'
...
export default function Home() {
  ...
  useEffect(() => {
    async function load() {
      const [trendsRes, bountiesRes, sightingsRes] = await Promise.all([
          ...
          <div className="flex flex-wrap gap-2">
            {trends.map((trend) => (
              <Link
                ...
                className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:border-brand-300 hover:bg-brand-50"
              >
                {trend.name}
              </Link>
            ))}
